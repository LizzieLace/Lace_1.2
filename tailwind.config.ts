module.exports = {
  content: ["./client/index.html", "./client/src/**/*.{js,jsx,ts,tsx}",
    "./src/**/*.{html,js,ts,jsx,tsx}",
    "app/**/*.{ts,tsx}",
    "components/**/*.{ts,tsx}",
  ],
  theme: {
    extend: {
      colors: {
        graysgray: "var(--graysgray)",
        "graysgray-5": "var(--graysgray-5)",
        "vibrant-labelsvibrant-secondary":
          "var(--vibrant-labelsvibrant-secondary)",
        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        background: "hsl(var(--background))",
        foreground: "hsl(var(--foreground))",
        primary: {
          DEFAULT: "hsl(var(--primary))",
          foreground: "hsl(var(--primary-foreground))",
        },
        secondary: {
          DEFAULT: "hsl(var(--secondary))",
          foreground: "hsl(var(--secondary-foreground))",
        },
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        muted: {
          DEFAULT: "hsl(var(--muted))",
          foreground: "hsl(var(--muted-foreground))",
        },
        accent: {
          DEFAULT: "hsl(var(--accent))",
          foreground: "hsl(var(--accent-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
      },
      fontFamily: {
        "avatar-initialsmd": "var(--avatar-initialsmd-font-family)",
        "avatar-initialssm": "var(--avatar-initialssm-font-family)",
        "body-small": "var(--body-small-font-family)",
        "caption2-regular": "var(--caption2-regular-font-family)",
        "chip-label": "var(--chip-label-font-family)",
        "footnote-regular": "var(--footnote-regular-font-family)",
        "m3-body-small": "var(--m3-body-small-font-family)",
        "m3-label-large-emphasized":
          "var(--m3-label-large-emphasized-font-family)",
        "m3-label-small": "var(--m3-label-small-font-family)",
        "m3-title-medium": "var(--m3-title-medium-font-family)",
        "text-sm-leading-5-normal":
          "var(--text-sm-leading-5-normal-font-family)",
        "text-sm-leading-6-medium":
          "var(--text-sm-leading-6-medium-font-family)",
        "text-xs-leading-4-semibold":
          "var(--text-xs-leading-4-semibold-font-family)",
        "ador-hairline": ["Ador Hairline", "sans-serif"],
        sans: [
          "ui-sans-serif",
          "system-ui",
          "sans-serif",
          '"Apple Color Emoji"',
          '"Segoe UI Emoji"',
          '"Segoe UI Symbol"',
          '"Noto Color Emoji"',
        ],
      },
      boxShadow: { "drop-shadow-100": "var(--drop-shadow-100)" },
      borderRadius: {
        lg: "var(--radius)",
        md: "calc(var(--radius) - 2px)",
        sm: "calc(var(--radius) - 4px)",
      },
      keyframes: {
        "accordion-down": {
          from: { height: "0" },
          to: { height: "var(--radix-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--radix-accordion-content-height)" },
          to: { height: "0" },
        },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
      },
    },
    container: { center: true, padding: "2rem", screens: { "2xl": "1400px" } },
  },
  plugins: [require("tailwindcss-animate"), require("@tailwindcss/typography")],
  darkMode: ["class"],
};
